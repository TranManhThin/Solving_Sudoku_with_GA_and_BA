...
public class SudokuGeneticAlgorithm {
	...
	private ArrayList<ArrayList<Integer>> giaiPhapBA = new ArrayList<>();
...
//		double reCombRate = ConstantGA.TI_LE_LAI_GHEP.getGiaTri();
	...
	private Chromosome laiGhepPMX(ArrayList<Chromosome> chaMe) {
		...
		con.setDoThichNghi(con.tinhDoThichNghi(this.kichThuocSudoku,giaiPhapBA));
		return con;
	}
...
//			caTheDotBien = dotBienHoanDoi(caThe);
	...
	private Chromosome dotBienHoanDoi(Chromosome caThe) {
		...
		caTheDotBien.setDoThichNghi(caTheDotBien.tinhDoThichNghi(this.kichThuocSudoku,giaiPhapBA)); // tính toạn độ thích nghi của cá thể vừa đột
																			// biến
	...
	private Chromosome dotBienXaoTron(Chromosome caThe) {
		...
		caTheDotBien.setDoThichNghi(caTheDotBien.tinhDoThichNghi(this.kichThuocSudoku,giaiPhapBA));// tính toán độ thích nghi của cá thể
	...
	public void khoiTaoQuanTheBanDau() {
		...
		System.out.println("Khởi tạo quần thể...");
		for (int count = 0; count <= this.soLuongQT; count++) {	
			Chromosome caThe = new Chromosome(this.kichThuocSudoku ,this.cauHoiSudoku, giaiPhapBA);
				this.quanThe.add(caThe);// thêm cá thể vào quần thể
				this.doThichNghiQuanThe.add(caThe.getDoThichNghi());// Thêm vào danh sách các giá trị thích nghi mỗi cá thể
			
		}
//		for(int i=0;i<this.soDao;i++) {
//			ArrayList<Chromosome> quanThe = new ArrayList<>();
//			ArrayList<Integer> doThichNghi = new ArrayList<>();
//			for(int j=0;j<this.soLuongQT;j++) {
//				Chromosome caThe = new Chromosome(this.kichThuocSudoku ,this.cauHoiSudoku, giaiPhapBA);
//				quanThe.add(caThe);
//				doThichNghi.add(caThe.getDoThichNghi());
//			}
//			tapHopDoThichNghiQuanTheDao.add(doThichNghi);
//			tapHopQuanTheDao.add(quanThe);
//		}
	}
...
//						System.out.println("Final : " + quanTheDao.get(findPosition));
	...
	public void trienKhaiGiaiThuatDiTruyen() {
		...
		for (int chuKy = 0; chuKy < this.vongDoi; chuKy += this.vongDoiMoiDao) {
			for (int dao = 0; dao < (int)ConstantGA.SO_DAO.getGiaTri(); dao++) {
	...
	public void trienKhaiBacktracking() {
		...
		for (int i = 0; i < this.kichThuocSudoku; i++) {
			...
			giaiPhapBA.add(hang);
			for (int j = 0; j < this.kichThuocSudoku; j++) {
				giaiPhapBA.get(i).set(j, this.cauHoiSudoku.get(i).get(j));
			}
		}
		int kichThuoc = this.kichThuocSudoku;
		BA ba = new BA(giaiPhapBA, kichThuoc);
		ba.giaiQuyet();
		giaiPhapBA = ba.ketQua();
	}
